...
from ..items import Clothing_Item


class Clothing_Spider(CrawlSpider):
    name = "clothing_spider"
    allowed_domains = ["junaidjamshed.com"]
    start_urls = ["https://www.junaidjamshed.com/"]
    country_code = "Pakistan" 
    
    rules = [
        Rule(LinkExtractor(allow="https://www.junaidjamshed.com/", restrict_css="ul.columns-group li.megamenu.level1 a[href*='/mens']", attrs="href"), 
            follow=True),
        Rule(LinkExtractor(restrict_css="a.product.photo.product-item-photo", attrs="href"), callback="parse_clothing_product", follow=True),
